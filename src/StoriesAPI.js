...
class StoriesAPI {
  ...
  async updateStories(userId, userData, dataType = 'form'){
    // validate
  ...
  async getUserStores(userId){
    ...
    // fetch the json data
    const response = await fetch(`${App.apiBase}/users/${userId}/stories`, {
      headers: { "Authorization": `Bearer ${localStorage.getItem("accessToken")}`}
    })
  ...
  async generateStory(userId, userData, dataType = 'form'){
    // validate
    if(!userId || !userData) return
    ...
    // make fetch request to backend
    const response = await fetch(`${App.apiBase}/users/${userId}/stories`, responseHeader)
